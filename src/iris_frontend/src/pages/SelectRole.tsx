...
const SelectRole: React.FC = () => {
  ...
  useEffect(() => {
    if (userRole && !hasNavigated) {
      console.log(
        `Navigating to ${
          userRole === "Merchant" ? "/input-merchant-name" : "/customer"
        }`
      );
      setHasNavigated(true); 
      navigate(userRole === "Merchant" ? "/input-merchant-name" : "/customer", {
        replace: true,
      });
    }
  }, [userRole, hasNavigated, navigate]);
    ...
    try {
      ...
      if (error.message && error.message.includes("already registered")) {
        ...
        if (profile) {
          ...
          console.error("No profile found for already registered user");
          alert("User is registered but no profile found. Please try again.");
        }
      } else {
  ...
  return (
    <div className="min-h-screen w-full bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-md bg-white/90 backdrop-blur-xl rounded-3xl shadow-xl px-8 py-10 text-center">
        <img
          src="/Logo_IRIS_gradient.svg"
          alt="Logo"
          className="w-48 h-24 mx-auto mb-4"
        />
        ...
        <div className="space-y-4 mb-6 text-left">
          ...
          >
            <div className="flex items-center">
              ...
              >
                <svg
                  ...
                  className={`h-5 w-5 ${
                    hoveredRole === "Merchant" ? "text-white" : "text-blue-500"
                  }`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className={`h-5 w-5 ${
                hoveredRole === "Merchant" ? "text-white" : "text-gray-400"
              }`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
          ...
          >
            ...
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className={`h-5 w-5 ${
                hoveredRole === "Customer" ? "text-white" : "text-gray-400"
              }`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
